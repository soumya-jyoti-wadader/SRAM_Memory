...
# Simulation defines for foundry delay macros (no-op in functional sim)
IVERILOG_BUILD_ARGS = ["-g2005-sv", "-DPOR_HLD_DELAY=", "-DPOR_MEM_DELAY="]


async def apply_reset(dut, cycles: int = 2) -> None:
    """Hold global_reset active, then release."""
...
async def sram_write(dut, addr: int, data_bit: int) -> None:
    """Perform one synchronous write (data_in is 1-bit; COLS=8 with full write mask)."""
    dut.addr.value = addr
...
def memory_word(dut, addr: int) -> int:
    """Read back a memory location through hierarchy (data_out is not driven in RTL)."""
    return int(dut.memory[addr].value)


@cocotb.test()
async def test_reset_deasserts_write_strobe(dut):
    """After reset, internal write-delay pipeline should be idle."""
...
async def test_single_write(dut):
    """Write a 1 to bit 0 at address 0 (zero-extended 1-bit data_in)."""
    clk = Clock(dut.clk, 10, unit="us")
    clk_inst = Clock(dut.clk_inst, 10, unit="us")
    clk.start(start_high=False)
    clk_inst.start(start_high=False)
...
async def test_random_writes(dut):
    """Random single-bit writes to several addresses."""
    clk = Clock(dut.clk, 10, unit="us")
    clk_inst = Clock(dut.clk_inst, 10, unit="us")
    clk.start(start_high=False)
    clk_inst.start(start_high=False)
    ...
    for _ in range(8):
        ...
        expected[addr] = bit  # full mask => only column 0 receives data_in
...
def test_sram_hidden_runner():
    ...
    sources = [
        ...
        proj_path / "golden/timescale.v",
        proj_path / "golden/sram.sv",
    ]
